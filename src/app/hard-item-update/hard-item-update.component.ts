...
@Component({
  selector: 'app-hard-item-update',
  templateUrl: './hard-item-update.component.html',
  styleUrls: ['./hard-item-update.component.css']
})
export class HardItemUpdateComponent implements OnInit {
  ...
  searchByName(){
    ...
    this.showLoading();
    this.fireStore.collection('physical_books', ref => ref.where('name','==',name).limit(1))
    .get()
    .toPromise()
    .then((querySnapshot)=>{
  ...
  private setFormWithData(book:HardItem):void{
    ...
    this.deliveryCharge.setValue(book.getDeliveryCharge());
    this.coverUrl.setValue(book.getPicUrl());
    this.type.setValue(book.getType());
    this.priority.setValue(book.getPriority().toString());
    
    this.discount.setValue(book.getDiscount());
    this.available.setValue(book.isAvailable()?'true':'false');
    this.merchant.setValue(book.getMerchant());
    this.material.setValue(book.getMaterial());
    this.stock.setValue(book.getStock());
    this.isBook.setValue(book.isIsBook()?'true':'false')


    this.searchKeywords = book.getSearchKeywords() || [];
    this.allPictures = book.getAllPicsUrls() || [];
